...
class EnhancedResNet(nn.Module):
    def __init__(self, resnet_type, enc_dim, nclasses):
        ...
        self.resnet = ResNet(1, enc_dim, resnet_type=resnet_type, nclasses=nclasses)
        self.self_enhancement = SelfEnhancementModule(enc_dim)
        self.mutual_enhancement = MutualEnhancementModule(enc_dim)

    def forward(self, x, other_branch_features=None):
        features = self.resnet(x)
        enhanced_features = self.self_enhancement(features)
        if other_branch_features is not None:
            enhanced_features, other_branch_features = self.mutual_enhancement(enhanced_features, other_branch_features)
...
def train(args):
    ...
    enhanced_resnet_lfcc = EnhancedResNet('18', args.enc_dim, 2).to(args.device)
    enhanced_resnet_cqt = EnhancedResNet('18', args.enc_dim, 2).to(args.device)
    ...
    resnet_lfcc_optimizer = torch.optim.Adam(enhanced_resnet_lfcc.parameters(), lr=args.lr, betas=(args.beta_1, args.beta_2), eps=args.eps, weight_decay=1e-4)
    resnet_cqt_optimizer = torch.optim.Adam(enhanced_resnet_cqt.parameters(), lr=args.lr, betas=(args.beta_1, args.beta_2), eps=args.eps, weight_decay=1e-4)
    ...
    for epoch_num in range(args.num_epochs):
        ...
        enhanced_resnet_lfcc.train()
        enhanced_resnet_cqt.train()
        ...
        for i, (lfcc, cqt, label, fakelabel) in enumerate(tqdm(trainDataLoader)):
            ...
            # LFCC branch
            feature_lfcc = enhanced_resnet_lfcc(lfcc)
            ...
            fcloss_lfcc = criterion(feature_lfcc, label)

            # CQT branch
            feature_cqt = enhanced_resnet_cqt(cqt)
            ...
            fcloss_cqt = criterion(feature_cqt, label)
        ...
        enhanced_resnet_lfcc.eval()
        enhanced_resnet_cqt.eval()
        classifier_cqt.eval()
        ...
        with torch.no_grad():
            ...
            for i, (lfcc, cqt, label, _) in enumerate(tqdm(valDataLoader)):
                ...
                feature_lfcc = enhanced_resnet_lfcc(lfcc)
                fcloss_lfcc = criterion(feature_lfcc, label)
                score_lfcc = F.softmax(feature_lfcc, dim=1)[:, 0]

                feature_cqt = enhanced_resnet_cqt(cqt)
                fcloss_cqt = criterion(feature_cqt, label)
                score_cqt = F.softmax(feature_cqt, dim=1)[:, 0]
        ...
        torch.save(enhanced_resnet_lfcc, os.path.join(args.out_fold, 'checkpoint', 'anti-spoofing_lfcc_model_%d.pt' % (epoch_num + 1)))
        torch.save(enhanced_resnet_cqt, os.path.join(args.out_fold, 'checkpoint', 'anti-spoofing_cqt_model_%d.pt' % (epoch_num + 1)))

    return enhanced_resnet_lfcc
